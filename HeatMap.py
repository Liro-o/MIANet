    ...
    def __call__(self, img_path, save_path):
        ...
        for i in trange(int(post_result.size(0) * self.ratio)):
            if float(post_result[i].max()) < self.conf_threshold:
                print("float(post_result[i].max())", float(post_result[i].max()))
                break
...
def get_params():
    params = {
        'weight': 'F:/lic_files/MogaNet/runs/detect/train88/weights/best.pt',  # 训练出来的权重文件
        'cfg': 'F:/lic_files/MogaNet/ultralytics/cfg/models/MogaNet.yaml',  # 训练权重对应的yaml配置文件
        'device': 'cuda:1',
...
if __name__ == '__main__':
    ...
    model(r'H:/HeatMapOriginData/00000516.png', ## 25 136 426 478 516
          'H:/HeatMapOriginData/Results/516')  # 第一个是检测的文件, 第二个是保存的路径
